...
```{r echo=FALSE}
knitr::read_chunk("${CYTOFPIPE_HOME}/cytofpipe.R")
```
...
if(length(autogating)==1){
	...
	if(autogating == "yes"){
		...
		cat("  \n**Gating template**  \n")
		cat("\\newline")
		cat(paste0("![Test]( ",outputdir,"/Gating/gating_scheme.pdf)"))
		cat("\\newline")
		cat("\\newpage")
		...
		for(i in 1:length(files_short)){
			...
			cat("\\newline")
			cat(paste0("![Test]( ",outputdir,"/Gating/gating_",files_short[i],".pdf)"))
			cat("\\newline")
			cat("\\newpage")
		}
...
library(cytofkit)
load(paste0(outputdir, "/cytofpipe.RData"))
			...
			if(!is.null(dataj)){
				for(i in 1:length(visualizationData)){
					if(!is.null(visualizationData[[i]])){
						...
						cat(paste0("  \n**",methodi," ",methodj, "**  \n"))
						cat("\\newline")
						cat(paste0("![Test]( ",outputdir,"/", methodj,"/cytofpipe_",methodi,"_",methodj,"_cluster_scatter_plot.pdf)"))
						cat("\\newline")
						cat("\\newpage")
					}
				}

				cat(paste0("  \n**",methodj, " Mean (scaled) Heatmap**  \n"))
				cat("\\newline")
				cat(paste0("![Test]( ",outputdir,"/", methodj,"/cytofpipe_",methodj,"_cluster_mean_heatmap_scaled.pdf)"))
				cat("\\newline")
				cat("\\newpage")                          
			}
		}
