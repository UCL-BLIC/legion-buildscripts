...
print OUTF "singularity exec -B /lustre/scratch/scratch/$ENV{USER} $ENV{CYTOFPIPE_HOME}/container/cytofpipe.img cytofpipe.pl "."@ARGV"."\n";
for (my $iter_i=2;$iter_i<=$iter;$iter_i++){
	$ARGV[($index_out+1)]=$outputdir."_".$iter_i;
	print OUTF "singularity exec -B /lustre/scratch/scratch/$ENV{USER} $ENV{CYTOFPIPE_HOME}/container/cytofpipe.img cytofpipe.pl "."@ARGV"." --array\n";
}
